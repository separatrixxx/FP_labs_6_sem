...
let parse tokens = 
    ...
    let keywords = ["lt"; "is"; "mut"; "spell"; "cast"; "if"; "else"; "readFile"; "writeFile"]
    ...
    let rec token_parser acc = function
        ...
        | Token.LEFT_PARENTHESIS::t ->
            ...
            | Expr.SIMPLE("lt")::Expr.SIMPLELIST([Expr.ID(id)])::Expr.SIMPLE(":")::Expr.SIMPLELIST(list)::[] -> token_parser (Expr.SIMPLELIST([Expr.VARIABLE(id, Expr.SIMPLELIST(list))])::acc) remaining_part
            | Expr.SIMPLE("mut")::Expr.SIMPLELIST([Expr.ID(id)])::Expr.SIMPLE(":")::Expr.SIMPLELIST(list)::[] -> token_parser (Expr.SIMPLELIST([Expr.SET(id, Expr.SIMPLELIST(list))])::acc) remaining_part

            | Expr.SIMPLE("spell")::Expr.SIMPLELIST([Expr.ID(id)])::(Expr.SIMPLEARGLIST(args_list) as args)::Expr.SIMPLE("=>")::(Expr.SIMPLELIST(_) as body)::[] -> 
